...
The protocol issues an over‑collateralized synthetic USD asset (`dfUSD`) and wires it into the Euler Vault Kit (EVK) stack to support collateralized borrowing, modular interest rate policies, and operational automation via a Gnosis Safe.

## What you can do with the protocol

* **Borrow `dfUSD` against collateral** using EVK vaults (e.g. WETH, WBTC) once governance has enabled them as collateral for the `dfUSD` debt vault.
...
* [Contracts reference](gitbook/contracts/)
