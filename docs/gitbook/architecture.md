...
### 1) Synthetic asset: `nUSD`

`nUSD` is an ERC‑20 with role‑based permissions:

- `MINTER_ROLE`: entities that can mint `nUSD` (e.g. the PSM)
- `ALLOCATOR_ROLE`: entities that can allocate `nUSD` into ERC‑4626 vaults (protocol treasury operations)
- `KEEPER_ROLE`: entities that can move accrued interest into the Savings Rate Module

See `docs/gitbook/contracts/nusd.md`.
...
- Borrow/repay of the debt asset (`nUSD`)
...
- An interest rate model (IRM) on the `nUSD` vault

Configuration of collateral LTVs is performed via `ops/src/propose-synth-collateral.ts`.

### 3) Peg Stability Module (PSM)

The PSM is a swap module that allows users to mint/burn `nUSD` against an underlying ERC‑20 (e.g. USDC) at a configured
...
`SavingsRateModule` is an ERC‑4626 vault for `nUSD` that streams donated `nUSD` over time to depositors. It is intended
...
flowchart LR
  ...
  User -->|borrow nUSD| DebtVault --> nUSD --> User

  User -->|swap USDC→nUSD| PSM --> nUSD
  User -->|swap nUSD→USDC| PSM
